...
protected:
	...
	bool willBeOccupied(const sf::Vector2i& pos) const; // git
	const int merge(const sf::Vector2i& pos, const Taction& direction); // nadpisane
	const int findFreeSpace(const sf::Vector2i& pos, const Taction& direction); // git
...
public:
	...
	TileBoard();
	TileBoard(const size_t& width, const size_t& height);
	TileBoard(const TileBoard& other);
	~TileBoard();
};